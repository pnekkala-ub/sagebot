...
from services.facts import summarize, fetchFacts
...
def chat():
    ...
    chitchat_response = chitchat_generator.generate_response(
        user_message, dialog_manager_response['context'])
    fact_response = fetchFacts(" ".join(dialog_manager_response["context"]))
    # fact_response = fact_responses[0]
    # fact_response = ''
    ...
    for ind, redd_response in enumerate(reddit_responses):
        reranked_responses = rerank(dialog_manager_response["intent_probs"], [chitchat_response, reddit_responses[0], empathetic_dialog_response], user_message)
        ...
        if reranked_responses[0] == redd_response: # if best reddit response
            if ind == len(reddit_responses)-1:
                ...
                if fact_response:
                    reranked_responses = rerank([dialog_manager_response["intent_probs"][1]], [fact_response], user_message)
                print("Topic Drift " + "user_query_topic: " + user_query_topic + "current_topic: " + current_topic)
                break
            if current_topic == user_query_topic: break
            else:
                continue
        else:
            break
    ...
    return {'chat_bot_response': chat_bot_response,
